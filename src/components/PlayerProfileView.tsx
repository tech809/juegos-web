...
export default function PlayerProfileView({ game }: { game: GameId }) {
  ...
  const legacyGames = stats.legacy_games ?? 0;

  return (
    <div className="space-y-8">
      <Link href={`${base}/jugadores`} className="text-xs font-display uppercase tracking-wide opacity-60 hover:opacity-100">
        ← Volver a Jugadores
      </Link>
      ...
      <div className="ornate rounded-sm p-5 sm:p-8 flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
        ...
        <div>
          ...
          <p className="text-sm opacity-70">
            <CountUp value={stats.wins} /> victorias de <CountUp value={stats.games_played} />{" "}
            {isMus ? "partidas" : "batallas"} · {ratePercent}% de gloria
          </p>
          {legacyGames > 0 && (
            <p className="text-[11px] opacity-50 italic mt-0.5 flex items-center gap-1 justify-center sm:justify-start">
              <ScrollIcon className="w-3 h-3" />
              incluye {legacyGames} de temporadas anteriores a la app
            </p>
          )}
          {badges.length > 0 && (
      ...
      <div>
        ...
        {games.length === 0 ? (
          <p className="text-sm opacity-60 italic">Sin partidas registradas todavía.</p>
        ) : (
          <ul className="space-y-2">
            {games.map((g) => {
              const myTeam = g.players.find((p) => p.id === player.id)?.team;
              const won =
                isMus && g.winner_team !== null && g.winner_team !== undefined
                  ? myTeam === g.winner_team
                  : g.winner_id === player.id;
              const isWinnerPlayer = (p: { id: string; team?: number | null }) =>
                isMus && g.winner_team !== null && g.winner_team !== undefined
                  ? p.team === g.winner_team
                  : p.id === g.winner_id;
              return (
                <li
                  key={g.id}
                ...
                >
                  {g.image && (
                    ...
                    <img src={g.image} alt="Foto de la partida" className="w-16 shrink-0 object-cover" />
                  )}
                    ...
                    <div className="flex flex-wrap items-center gap-1 text-xs font-display opacity-80">
                      {g.players.map((p, i) => (
                        <span key={p.id} className="flex items-center gap-1">
                          ...
                          {isWinnerPlayer(p) && <CrownIcon className="w-3 h-3 text-gold" />}
