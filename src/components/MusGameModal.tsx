...
import { CameraIcon, CardsIcon, CheckIcon, ChevronDownIcon, CrownIcon, PencilIcon, SearchIcon, SwordsIcon, XIcon } from "./icons";
import Skeleton from "./Skeleton";
import ImageEditModal from "./ImageEditModal";
import PlayerAvatar from "./PlayerAvatar";

export default function MusGameModal({
  onClose,
  onSaved,
}: {
  onClose: () => void;
  onSaved: (winnerNames: string) => void;
}) {
  ...
  async function createAndSelect(name: string) {
    ...
    try {
      ...
      setSelectedIds((prev) => (prev.length >= 4 || prev.includes(player.id) ? prev : [...prev, player.id]));
      setSearch("");
    } catch {
  ...
  async function confirm() {
    ...
    try {
      ...
      const winners = winnerTeam === 0 ? teamA : teamB;
      onSaved(winners.map((p) => p.name).join(" y "));
    } catch {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-xl font-bold flex items-center gap-2">
              <CardsIcon className="w-5 h-5 text-wine" />
          ...
          <section className="mb-5 relative" ref={playersRef}>
            <h3 className="text-xs font-display font-semibold uppercase tracking-[0.15em] mb-2 opacity-70">
              Elige a los cuatro jugadores ({selectedIds.length}/4)
            </h3>
                  ...
                  <div className="ornate bg-card rounded-sm overflow-hidden">
                    ...
                    {loading ? (
                      ...
                      <div className="max-h-52 overflow-y-auto py-1">
                        ...
                        {full && (
                          <p className="text-[11px] opacity-50 italic px-3 py-2">Máximo 4 jugadores por partida.</p>
                        )}
                      </div>
                    )}
                  </div>
                          ...
                          {[0, 1].map((slot) => {
                            ...
                            return p ? (
                              <div key={p.id} className="flex items-center gap-2 bg-black/5 rounded px-2 py-1">
                                <PlayerAvatar name={p.name} color={p.color} photo={p.photo} size={24} bordered />
